...
namespace graphchi {
    ...
    public:
        ...
        void first_iteration(graphchi_context &gcontext_) {
            this->set_data(kernel.init(gcontext_, vinfo));
        }
        ...
        inline void add_inedge(vid_t src, ET *ptr, bool special_edge) {
            if (gcontext->iteration > 0) {
                auto val = kernel.gather(*gcontext, vinfo, src, *ptr);
#ifdef HTM
        ...
        inline void add_outedge(vid_t dst, ET *ptr, bool special_edge) {
            *ptr = kernel.scatter(*gcontext, vinfo, dst, this->get_data());
        }
    ...
    class FunctionalProgramProxySemisync
            : public GraphChiProgram<typename KERNEL::VertexDataType, typename KERNEL::EdgeDataType, functional_vertex_unweighted_semisync<KERNEL> > {
    public:
