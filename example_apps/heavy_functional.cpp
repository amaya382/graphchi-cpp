...
struct heavy_kernel : public functional_kernel<float, float> {
...
int main(int argc, const char ** argv) {
    ...
    if (onlytop == 0) {
        ...
        if (mode == "semisync") {            
            ...
            run_functional_unweighted_semisynchronous<heavy_kernel>(filename, niters, m);
        } else if (mode == "sync") {
            logstream(LOG_INFO) << "Running pagerank in (bulk) synchronous mode." << std::endl;
            run_functional_unweighted_synchronous<heavy_kernel>(filename, niters, m);
        } else {
